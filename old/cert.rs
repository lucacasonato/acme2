...
impl<'a> CertificateSigner<'a> {
  ...
  pub async fn sign_certificate(
    ...
    while order_response.status == "processing"
      || order_response.certificate.is_none()
    {
...
impl SignedCertificate {
  ...
  pub async fn save_signed_certificate<P: AsRef<Path>>(
    &self,
    path: P,
  ) -> Result<()> {
  ...
  pub async fn write_private_key<W: AsyncWrite>(
    &self,
    mut writer: Pin<&mut W>,
  ) -> Result<()> {
    writer
  ...
  pub async fn write_csr<W: AsyncWrite>(
    &self,
    mut writer: Pin<&mut W>,
  ) -> Result<()> {
